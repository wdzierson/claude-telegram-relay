...
var PROJECT_ROOT = dirname(dirname(dirname(import.meta.path)));
...
function loadConfig() {
  const relayDir = process.env.RELAY_DIR || join(process.env.HOME || "~", ".bright");
  const telegram = {
    botToken: requiredEnv("TELEGRAM_BOT_TOKEN"),
    allowedUserIds: process.env.TELEGRAM_USER_ID ? process.env.TELEGRAM_USER_ID.split(",").map((id) => id.trim()) : []
  };
  const claude = {
  ...
  const agentBackend = process.env.AGENT_BACKEND || "cli";
  let anthropic;
  ...
  const heartbeat = {
    ...
    checkinIntervalMs: parseInt(process.env.HEARTBEAT_CHECKIN_INTERVAL_MS || "1800000"),
  ...
  return { agentBackend, telegram, claude, anthropic, supabase, voice, tts, tasks, queue, heartbeat, location, mcp, user, paths };
}
...
async function loadProfile(projectRoot) {
  try {
    return await readFile(join2(projectRoot, "config", "profile.md"), "utf-8");
  } catch {
    return "";
  }
}
...
class RealtimeChannel {
  ...
  _trigger(type, payload, ref) {
    ...
    const { close, error, leave, join: join3 } = CHANNEL_EVENTS;
...
async function navigatorLock(name, acquireTimeout, fn) {
  ...
  try {
    ...
    if ((e === null || e === undefined ? undefined : e.name) === "AbortError") {
...
async function processMemoryIntents(supabase, response, userId) {
  if (!supabase)
    return response;
  let clean = response;
  ...
  for (const match of response.matchAll(/\[DONE:\s*(.+?)\]/gi)) {
    const query = supabase.from("memory").select("id").eq("type", "goal").ilike("content", `%${match[1]}%`).limit(1);
    ...
    async getMemoryContext(userId) {
      ...
      try {
        ...
        if (factsResult.data?.length) {
          parts.push(`FACTS:
` + factsResult.data.map((f) => `- ${f.content}`).join(`
`));
        }
    ...
    async getRelevantContext(query, userId) {
      ...
      try {
        const [messagesResult, memoryResult] = await Promise.allSettled([
          ...
          client.functions.invoke("search", {
            body: { query, match_count: 5, table: "memory" }
          })
        ]);
        const parts = [];
        if (messagesResult.status === "fulfilled" && messagesResult.value.data?.length) {
          parts.push(`RELEVANT PAST MESSAGES:
` + messagesResult.value.data.map((m) => `[${m.role}]: ${m.content}`).join(`
`));
        }
        if (memoryResult.status === "fulfilled" && memoryResult.value.data?.length) {
          parts.push(`RELEVANT MEMORY:
` + memoryResult.value.data.map((m) => `[${m.type}]: ${m.content}`).join(`
`));
        }
        return parts.join(`
...
class APIPromise extends Promise {
  ...
  _thenUnwrap(transform) {
    return new APIPromise(__classPrivateFieldGet2(this, _APIPromise_client, "f"), this.responsePromise, async (client, props) => addRequestID(transform(await this.parseResponse(client, props), props), props.response));
  }
  asResponse() {
            ...
            if (snapshotContent && tracksToolInput(snapshotContent)) {
              ...
              if (jsonBuf) {
                try {
                  ...
                  __classPrivateFieldGet2(this, _BetaMessageStream_handleError, "f").call(this, error2);
                }
              }
    ...
    try {
      while (true) {
        ...
        try {
          ...
          __classPrivateFieldSet2(this, _BetaToolRunner_toolResponse, undefined, "f");
          __classPrivateFieldSet2(this, _BetaToolRunner_iterationCount, (_a = __classPrivateFieldGet2(this, _BetaToolRunner_iterationCount, "f"), _a++, _a), "f");
      ...
      __classPrivateFieldGet2(this, _BetaToolRunner_completion, "f").reject(error2);
      __classPrivateFieldSet2(this, _BetaToolRunner_completion, promiseWithResolvers(), "f");
      throw error2;
    }
  }
...
async function callAnthropicAPI(systemPrompt, userMessage, config) {
  ...
  try {
    ...
    console.error("Anthropic API error:", error2.message || error2);
    return `Error: ${error2.message || "Anthropic API call failed"}`;
  }
}

// src/scheduler/telegram.ts
var MAX_LENGTH = 4000;
async function sendTelegram(botToken, chatId, message, parseMode) {
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      console.error("Telegram send error:", error2);
      return false;
    }
  }
  return true;
...
async function createSchedulerContext() {
  ...
  return {
    ...
    memory,
    profile,
    sendTelegram: (message, parseMode) => sendTelegram(config.telegram.botToken, chatId, message, parseMode),
    callClaude: (systemPrompt, userMessage) => callAnthropicAPI(systemPrompt, userMessage, config.anthropic)
  };
}

// src/scheduler/data.ts
async function fetchGoals(client2) {
  try {
    const { data } = await client2.rpc("get_active_goals");
    return (data || []).map((g) => ({
      content: g.content,
      deadline: g.deadline ? new Date(g.deadline).toLocaleDateString() : undefined
    }));
  } catch (error2) {
    console.error("Goals fetch failed:", error2);
    return [];
  }
}
async function fetchFacts(client2) {
  try {
    const { data } = await client2.rpc("get_facts");
    return (data || []).map((f) => f.content);
  } catch (error2) {
    console.error("Facts fetch failed:", error2);
    return [];
  }
}
async function fetchActivitySummary(client2, timezone) {
  try {
    const { data: recentMessages } = await client2.rpc("get_recent_messages", {
      limit_count: 20
    });
    const now = new Date;
    ...
    if (recentMessages?.length) {
      ...
      hoursSinceLastMessage = (now.getTime() - new Date(lastMessageTime).getTime()) / (1000 * 60 * 60);
    ...
    console.error("Activity fetch failed:", error2);
    return {
...
async function fetchTaskSummary(client2) {
  try {
    const todayStart = new Date;
    todayStart.setHours(0, 0, 0, 0);
    const [activeResult, completedResult] = await Promise.all([
      client2.from("tasks").select("description, status").in("status", ["pending", "running"]),
      client2.from("tasks").select("description").eq("status", "completed").gte("updated_at", todayStart.toISOString())
    ]);
    return {
      activeTasks: (activeResult.data || []).map((t) => ({
        description: t.description,
        status: t.status
      })),
      completedToday: (completedResult.data || []).map((t) => ({
        description: t.description
      }))
    };
  } catch (error2) {
    console.error("Tasks fetch failed:", error2);
    return { activeTasks: [], completedToday: [] };
  }
}

// src/scheduler/checkin.ts
...
async function main() {
  ...
  const client2 = memory.client;
  const [goals, activity, tasks, facts] = await Promise.all([
    fetchGoals(client2),
    fetchActivitySummary(client2, config.user.timezone),
    fetchTaskSummary(client2),
    fetchFacts(client2)
  ]);
